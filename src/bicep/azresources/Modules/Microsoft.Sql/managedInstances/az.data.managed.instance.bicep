...
module managedInstance_roleAssignments './rbac/roleAssignments.bicep' = [for (roleAssignment, index) in roleAssignments: {
...
module managedInstance_databases 'databases/az.data.managed.instance.database.bicep' = [for (database, index) in databases: {
...
module managedInstance_securityAlertPolicy './securityAlertPolicies/az.data.managed.instance.security.alert.policy.bicep' = if (!empty(securityAlertPoliciesObj)) {
...
module managedInstance_vulnerabilityAssessment './vulnerabilityAssessments/az.data.managed.instance.vulnerability.assessment.bicep' = if (!empty(vulnerabilityAssessmentsObj)) {
...
module managedInstance_key './keys/az.data.managed.instance.key.bicep' = [for (key, index) in keys: {
...
module managedInstance_encryptionProtector './encryptionProtector/az.data.managed.instance.encryption.protector.bicep' = if (!empty(encryptionProtectorObj)) {
...
module managedInstance_administrator './administrators/az.data.managed.instance.administrator.bicep' = if (!empty(administratorsObj)) {
