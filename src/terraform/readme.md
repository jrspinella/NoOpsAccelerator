# Azure NoOps Accelerator - Setup Guide for Terraform
...
This guide describes how to setup your environment with the Azure NoOps Accelerator using the [Terraform](https://www.terraform.io/) template at [src/terraform/](../src/terraform/).
...
### What is a Mission Enclave?

A Mission Enclave is a collection of resources that are deployed together to support a specific mission. For example, a Mission Enclave may be a collection of resources and overlays that support a specific application or a collection of resources that support a specific business unit.

A Mission Enclave may be deployed to a single subscription or multiple subscriptions. A Mission Enclave may be deployed to a single region or multiple regions. A Mission Enclave may be deployed to a single Azure environment or multiple Azure environments.

### What is a Platform Landing Zone?

A Platform Landing Zone is a collection of resources that are deployed together to support a specific platform. For example, a Platform Landing Zone may be a collection of resources and overlays that support a specific application or a collection of resources that support a specific business unit.

### What is a Workload?

A Workload is a collection of resources that are deployed together to support a specific workload. For example, a Workload may be a collection of resources and overlays that support a specific application or a collection of resources that support a specific business unit.

### What is the Enclave support folder?

Enclave support is a collection of folders that support a deployment of an Mission Enclave.

The enclave support folders are:

- [enclaves](../src/terraform/enclaves/) - contains the Mission Enclaves
- [modules](../src/terraform/modules/) - contains the modules that are used by the Mission Enclaves
- [platforms](../src/terraform/platforms/) - contains the Platform Landing Zones that are used by the Mission Enclaves
- [workloads](../src/terraform/workloads/) - contains the Workloads that are used by the Mission Enclaves

> NOTE: Platform landing zones and Workloads can be deployed indepently from an enclave but are not intended to be deployed directly to Azure. They are intended to be used as modules that are deployed by Mission Enclaves. These examples are provided to help you understand how to use the Azure NoOps Accelerator.

### What is in the Example folder?

The [examples](../src/terraform/examples/) folder contains a collection of Mission Enclaves that can be deployed to Azure.



## Planning

### Decide on a Organization Prefix

Resource Groups and resource names are derived from the `org_prefix` parameter, which defaults to 'anoa'. Pick a unqiue organization prefix that is 3-10 alphanumeric characters in length without whitespaces.
...
`id_subid` | value of hub_subid | Subscription ID for tier 0 (Identity)
`ops_subid` | value of hub_subid | Subscription ID for tier 1 (Operations)
`svcs_subid` | value of hub_subid | Subscription ID for tier 2 (Shared Services)

### Networking

The following parameters affect networking addresses. To override the defaults edit the variables file at [`src/terraform/examples/enclaves/hub3spoke-aks/variables.tf`](../examples/enclaves/hub3spoke-aks/variables.tf).
...
`id_vnet_address_space` | `["10.0.110.0/26"]` | VNet prefix for tier 0
`id_subnets.address_prefixes` | `["10.0.110.0/27"]` | Subnet prefix for tier 0
`ops_vnet_address_space` | `["10.0.115.0/26"]` | VNet prefix for tier 1
`ops_subnets.address_prefixes` | `["10.0.115.0/27"]` | Subnet prefix for tier 1
`svcs_vnet_address_space` | `["10.0.120.0/26"]` | VNet prefix for tier 2
`svcs_subnets.address_prefixes` | `["10.0.120.0/27"]` | Subnet prefix for tier 2
...
1. Navigate to the directory in the repository that contains the Terraform module:

    ```bash
    cd src/terraform/examples/enclaves/hub3spoke-aks
    ```
...
ops_subid = "{the Tier 1 subscription ID}"
```

### Deploy New Workloads

Once you've deployed Mission Enclave, you can use the Tier 3 module to deploy and peer new Spoke Networks and workloads to the Hub and Firewall.
...
If you want to delete an Mission Enclave deployment you can use [`terraform destroy`](https://www.terraform.io/docs/cli/commands/destroy.html). If you have deployed more than one Terraform template, e.g., if you have deployed `examples/hub3spoke-aks/main.tf` and then `workloads/<other service>/main.tf`, run the `terraform destroy` commands in the reverse order that you applied them. For example:
