# Azure NoOps Accelerator - Deployment Guide for Terraform
...
Below is an example of a Terraform deployment of an Mission Enclave with Azure Kubernetes Cluster that uses all the defaults.
...
### Why are there Enclaves Example folders?

The Azure NoOps Accelerator is designed to be flexible and extensible. Since we want to deploy SCCA compliant environments, deploying Mission Enclaves allows to combine various platform landing zones and workloads that are deployed together to support a specific mission.

Platform landing zones and Workloads can be deployed indepently from an enclave but are not intended to be deployed directly to Azure. They are intended to be used as modules that are deployed by Mission Enclaves. These examples are provided to help you understand how to use the Azure NoOps Accelerator.
